...
// Author notation explicitly, keeping prose and readable fallbacks separate
...
export const displayMath=(tex,text=tex)=>({tex,text,display:true});
...
export function renderMathText(value){
  ...
  return value.map(part=>typeof part==='object'&&part!==null&&typeof part.tex==='string'
    ?`<span class="${part.display?'display-math':'inline-math'}" data-tex="${escapeHtml(part.tex)}" data-math-display="${part.display?'block':'inline'}">${escapeHtml(part.text)}</span>`
